---
title: Extract ICD code inclusion & exclusion criteria from tables in Gonzalez-Izquierdo et al. 2010, and Schnitzer et al 2011, and compare them
author: "Jan Savinc"
...
schnitzer_2004_icd9cm <-
  ...
  filter(str_starts(code_decimal, pattern="Diagnosis codes|External cause of", negate = TRUE)) %>%  # remove headings based on the phrases they begin with
  mutate(
    description_icd9 = gsub(code_decimal, pattern="^.*\\: (.*)$", replacement = "\\1"),  # extract description
    code_decimal = gsub(code_decimal, pattern="^\\s*(.*)\\:.*$", replacement = "\\1"),  # extract code
    inclusion_index = 1:nrow(.)
  )

# TODO; complete mapping schnitzer 2004 also!
...
schnitzer_2011_malnutrition_exclusions_icd9_icd10 <-
  ...
  mutate(
    icd9_range = case_when(
      ...
      code=="593.9" ~ "585.9",  # Schnitzer says Chronic renal insufficiency, which is actually ICD-9-CM 585.9
    ...
    icd10_range = case_when(
      ...
      code=="593.9" ~ "N18-N19",  # Schnitzer specified "Chronic renal insufficiency", but listed code 593.9 Disorders of kidney and ureter, unspecified; I'm assuming they mean the former, which maps to N18 & possibly N19
...
# TODO: compile a spreadsheet of mappings so others can check
## also write the condensed mapping as an excel file with a blank column for comments
# schnitzer_2011_mapping_for_excel <-
#   list(
#     "inclusion_codes" = schnitzer_2011_inclusions_empirical_icd9cm_and_icd9 %>% select(code_decimal,description_icd9cm,code_icd9,description_icd9, inclusion_index) %>% filter(!code_decimal %in% as.character(960:979)) %>% add_case(inclusion_index=36, code_decimal="960-979", code_icd9="960-979",description_icd9cm="Poisoning by drugs/medicinals", description_icd9="960-979 Poisoning by drugs, medicinal and biological substances") %>% arrange(inclusion_index) %>% left_join(schnitzer_2011_inclusions_icd10 %>% select(inclusion_index, icd10) %>% mutate(icd10=ifelse(inclusion_index==36,"T36-T50",icd10)) %>% distinct) %>% distinct %>% left_join(icd10_codes_UK %>% select(icd10=code,description_icd10=meaning)) %>%  mutate(comments_icd9=as.character(NA),comments_icd10=as.character(NA)) %>% select(code_icd9cm=code_decimal,description_icd9cm,description_icd9,comments_icd9,description_icd10,comments_icd10),
#     "exclusion_codes" = NULL,
#     "malnutrition" = NULL,
#     "addition_requirements_smoking" = NULL
#   )
# 
# openxlsx::write.xlsx(
#   
...
readable_spreadsheet_mapping <- list(
  ...
  maltreatment_exclusions = NULL, # TODO
